...
def print_players_games():
    ...
    print 'INSERT INTO PLAYERS (Gamertag, XUID)'
    print '    VALUES (\'Biglew117\', {});'.format(myXuid) 

    friends_list = get_friends(myXuid)
    for friend in friends_list:
        response = get_games(friend['id'])
        games_list = response['titles']
        for game in games_list:
            if game['titleType'].find('DGame') != -1:
                print 'INSERT INTO GAMES (GameTitle, GameID)'
                if game['name'].find('\'') != -1:
                    line = game['name'].encode('utf8') 
                    line = line.translate(None, '\'')
                    game['name'] = line
                    print('   VALUES (\'{}\', {});'.format(game['name'], game['titleId']))
                else:
                    print('   VALUES (\'{}\', {});'.format(game['name'].encode('utf8'), game['titleId']))
    return
